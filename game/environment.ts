...
const ink = '#101427';
function rect(c:C,x:number,y:number,w:number,h:number,fill:string,line=3) { c.fillStyle=fill;c.fillRect(x,y,w,h);if(line){c.strokeStyle=ink;c.lineWidth=line;c.strokeRect(x,y,w,h);} }
function path(c:C,points:number[][],fill:string,line=3){c.beginPath();points.forEach(([x,y],i)=>i?c.lineTo(x,y):c.moveTo(x,y));c.closePath();c.fillStyle=fill;c.fill();if(line){c.strokeStyle=ink;c.lineWidth=line;c.stroke();}}
function ellipse(c:C,x:number,y:number,rx:number,ry:number,fill:string){c.beginPath();c.ellipse(x,y,rx,ry,0,0,Math.PI*2);c.fillStyle=fill;c.fill();}
function text(c:C,s:string,x:number,y:number,size:number,color:string,align:CanvasTextAlign='left'){c.font=`900 ${size}px Arial, sans-serif`;c.fillStyle=color;c.textAlign=align;c.fillText(s,x,y);}
function neon(c:C,s:string,x:number,y:number,size:number,color:string){c.save();c.shadowColor=color;c.shadowBlur=14;text(c,s,x,y,size,color);c.restore();}
...
function crown(c:C,x:number,y:number,size:number){c.save();c.shadowColor='#8dff4c';c.shadowBlur=22;path(c,[[x,y],[x+size*.24,y+size*.35],[x+size*.5,y-size*.18],[x+size*.76,y+size*.35],[x+size,y],[x+size*.86,y+size*.7],[x+size*.14,y+size*.7]],'#b0ff61',2);c.restore();}
...
export class GameScene {
  ...
  draw(c:C,w:number,h:number,camera:Camera,p:Player,time:number,reduced:boolean,phase:string){
    ...
    rect(c,-100,627,5100,49,'#64606a');rect(c,-100,675,5100,12,'#242a3c');rect(c,-100,672,5100,4,'#aba08b',0);for(let i=0;i<51;i++)rect(c,i*110,640,1,30,'#393b4a',0);for(let i=0;i<34;i++)rect(c,i*175,748,70,4,'#99857055',0);
    ...
    car(c,685,'#79667c');lamp(c,890);shop(c,1150,300,'BANANA MART','#64765a');rect(c,1475,556,47,87,'#66636f');rect(c,1470,551,57,10,'#39394b');
    ...
    this.drawPlayer(c,p,time,reduced);

...
export class GameRuntime {
 ...
 objective(){if(this.state.scene==='stop')return {id:'leave',label:'Look around · city exit →',x:1020};return this.mission.next(this.state.flags);}
 ...
 updatePlay(dt:number,direction:number){this.state.playedSeconds+=dt;const p=this.player;const jumped=this.controller.update(p,this.state.scene,dt,direction);if(jumped){this.state.flags.add('jump-used');this.audio.tone(245,.12,.02);}if(direction)this.state.flags.add('moved');
  ...
  this.camera.worldWidth=this.state.scene==='stop'?1180:mission001.worldWidth;this.camera.update(dt,p,this.viewWidth,this.reduced);this.camera.zoom+=(1-this.camera.zoom)*Math.min(1,dt*3);

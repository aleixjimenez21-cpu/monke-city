...
import {phoneX,smooth} from './cinematic';
export class GameRuntime {
 cinematicStartX=0;cinematicCameraX=0;lastRing=-10;lastEngine=-10;
 phoneNear(){return this.state.scene==='street'&&this.state.flags.has('complete')&&!this.state.flags.has('phone-answered')&&Math.abs(this.player.x-phoneX)<100&&this.player.grounded;}
 ...
 init(){this.camera.descent=0;this.hasSave=this.store.load(this.state,this.player);}
 ...
 start(resume=false){if(!resume){this.state=new GameState();this.player=new Player();this.camera=new Camera();this.store.clear();this.hasSave=false;}this.state.paused=false;this.camera.descent=this.state.flags.has('ride-complete')&&!this.reduced?.65:0;this.camera.x=Math.max(0,this.player.x-this.viewWidth*.37);this.announce(resume&&this.player.x>1650?'THE OUTSKIRTS':'MONKE MOTEL',resume?'YOUR JOURNEY CONTINUES':'THE OUTSKIRTS · START: $7');this.setPhase(resume&&this.state.flags.has('ride-complete')?'ending':'arriving');}
 announce(title:string,sub:string){this.locationTitle=title;this.locationSub=sub;this.locationTime=3;}
 objective(){if(this.state.flags.has('complete')&&!this.state.flags.has('ride-complete'))return {id:'phone',label:'Answer the phone',x:phoneX};if(this.state.scene==='stop')return {id:'leave',label:'Look around · city exit →',x:1090};return this.mission.next(this.state.flags);}
 ...
 reset(){this.state=new GameState();this.player=new Player();this.camera=new Camera();this.lastRing=-10;this.lastEngine=-10;this.dialogue.close();this.collectibles.particles=[];this.hasSave=false;this.store.clear();this.notice='';this.locationTime=0;this.storyCooldown=0;this.lastDistrict='';this.setPhase('ready');}
 ...
 interact(){if(this.state.phase!=='playing'||this.state.paused)return;if(this.phoneNear()){this.state.flags.add('phone-answered');this.player.velocity=0;this.player.vy=0;this.cinematicStartX=this.player.x;this.cinematicCameraX=this.camera.x;this.locationTime=0;this.setPhase('cinematic');return;}const e=this.interactions.nearest(this.state,this.player);if(!e)return;this.player.velocity=0;this.state.flags.add('interacted');this.audio.tone(440,.12,.025);
 ...
 update(dt:number,direction:number){if(this.state.paused)return;this.time+=dt;if(!this.inspectionHeld)this.phaseTime+=dt;this.noticeTime=Math.max(0,this.noticeTime-dt);this.achievementTime=Math.max(0,this.achievementTime-dt);this.hitCooldown=Math.max(0,this.hitCooldown-dt);this.collectibles.update(dt);if(this.state.phase!=='dialogue')this.locationTime=Math.max(0,this.locationTime-dt);this.storyCooldown=Math.max(0,this.storyCooldown-dt);
  switch(this.state.phase){
   case'ready':this.camera.descent=0;this.camera.x=this.reduced?1600:1600+Math.sin(this.time*.08)*90;break;
case'cinematic':{const t=this.phaseTime;this.player.facing=1;this.player.x=this.cinematicStartX+(3215-this.cinematicStartX)*smooth((t-7.5)/1.7);this.player.step+=dt*(t>7.5&&t<9.2?5:0);const target=Math.max(0,3215-this.viewWidth*.5);this.camera.x=this.cinematicCameraX+(target-this.cinematicCameraX)*smooth(t/3);this.camera.zoom=this.reduced?1:1+.18*smooth(t/3)-.18*smooth((t-10)/5);this.camera.descent=this.reduced?0:.65*smooth((t-10)/5);if(t>3&&this.time-this.lastEngine>.25){this.audio.enginePitch(t<7?.3:t<10?.1:Math.min(1,(t-10)/3));this.lastEngine=this.time;}if(t>=15){this.state.flags.add('ride-complete');this.setPhase('ending');this.save();}break;}
case'loading':if(this.phaseTime>.65)this.setPhase('ready');break;
 ...
 updatePlay(dt:number,direction:number){if(this.state.flags.has('complete')&&!this.state.flags.has('phone-answered')&&this.time-this.lastRing>2){this.audio.tone(880,.2,.025);this.audio.tone(1174,.4,.015);this.lastRing=this.time;}this.state.playedSeconds+=dt;const p=this.player;const jumped=this.controller.update(p,this.state.scene,dt,direction);if(jumped){this.state.flags.add('jump-used');this.audio.tone(245,.12,.02);}if(direction)this.state.flags.add('moved');
  ...
  if(this.state.scene==='street'){
   ...
   if(!this.state.flags.has('complete')&&this.mission.shouldEnd(p.x,this.state.flags)){this.state.flags.add('complete');p.velocity=0;this.announce('MONKE CITY','DOWNTOWN ACCESS');this.tell('INCOMING CALL');this.audio.tone(523,.65,.035);this.save();}
  }

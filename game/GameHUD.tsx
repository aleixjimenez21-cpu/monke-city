...
export function GameHUD({view,onInteract}:{view:GameSnapshot;onInteract:()=>void}){
 ...
 <header className="hud"><div className="wallet"><span>NET WORTH</span><strong key={view.netWorth} className="cash-update">${view.netWorth}<span className="wallet-dot">●</span></strong><small>FICTIONAL GAME CASH</small></div><div className="mission"><span>MISSION #{mission001.id}</span><b>{mission001.name}</b><div className="objective">{exitStore?'← Return to the street':(back?'← ':'→ ')+(goal?.label??'Mission complete')}</div></div><div className="banana-count" title="Find all five fictional collectibles"><span>🍌</span> {view.bananas}<small> / 5</small></div></header>
 ...
 {view.near&&<button className="interact" onClick={onInteract}><kbd>E</kbd><span>{view.near.label}</span> ↗</button>}

...
export function drawStreetObjects(c:C,g:GameRuntime){
 ...
 rect(c,2360,468,82,175,'#234347',4);rect(c,2368,480,66,101,'#102a31',3);neon(c,'$RICH',2373,514,18,'#b8fb70');text(c,'OUR STORY',2374,551,9,'#ece2b1');rect(c,2384,593,31,7,'#b7db97');
 ...
 rect(c,3380,440,259,173,'#455647',5);rect(c,3369,430,280,28,'#687358');text(c,'MONKE CITY',3391,450,15,'#f3e3ae');text(c,'CITY TRAM · $12',3391,482,18,'#d8f6be');for(let i=0;i<3;i++){rect(c,3395+i*74,499,62,75,i===0?'#d4d4a7':'#809077',1);text(c,i===0?'GO':i===1?'12':'→',3404+i*74,521,16,'#253e35');rect(c,3404+i*74,540,42,3,'#536553',0);rect(c,3404+i*74,550,30,3,'#536553',0);}rect(c,3390,615,8,47,'#3c4148');rect(c,3615,615,8,47,'#3c4148');
 text(c,'THE CROWN IS EARNED.',4160,514,15,'#b9a1d6');text(c,'YOU’RE EARLY.',4180,544,12,'#a2b391');
 text(c,'CONSTRUCTION STREET',4440,520,18,'#efd391');
...
export function drawInterior(c:C,w:number,h:number,g:GameRuntime,scene:GameScene){
 ...
 rect(c,247,565,168,87,'#9b7b61');rect(c,248,433,165,121,'#5d6268',5);rect(c,260,445,122,91,'#1f373a');for(let i=0;i<12;i++)rect(c,265,449+i*7,112,1,'#9ad3ad22',0);neon(c,'START: $7',270,493,20,'#b8fb70');text(c,'ON AIR',278,518,10,'#dcdbbe');ellipse(c,397,470,6,6,'#e0c792');ellipse(c,397,503,6,6,'#ada5a0');path(c,[[306,433],[281,404],[309,431],[340,401]],'#4a535a',2);label(c,'MONKE TV',330,597);
 rect(c,477,404,205,167,'#283e47',5);crown(c,550,420,37);path(c,[[575,455],[519,502],[570,541],[646,499],[575,455]],'#547363',2);ellipse(c,519,502,8,8,'#b8fb70');ellipse(c,646,499,7,7,'#9d9da0');text(c,'MONKE CITY',509,559,17,'#cfdbc2');label(c,'CITY MAP',580,599);
 ...
 rect(c,1088,455,124,200,'#586877',4);rect(c,1097,465,106,109,'#163a34');text(c,'SEASON 0',1109,498,16,'#b8fb70');text(c,'ACTIVE',1120,524,15,'#b8fb70');rect(c,1107,545,81,3,'#6da587',0);rect(c,1105,585,85,14,'#9ca296');ellipse(c,1150,619,8,8,'#b8fb70');label(c,'COMMUNITY BOARD',1150,432);

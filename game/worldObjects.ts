...
export function drawStreetObjects(c:C,g:GameRuntime){
 ...
 npc(c,1000,t);npc(c,2830,t,true,g.dialogue.encounter?.id==='ape'?g.dialogue.node?.effect:undefined);
 rect(c,1357,486,60,147,'#223b34');neon(c,'OPEN',1366,515,16,'#c2f192');label(c,'ENTER ↗',1385,580,'#dcdbbb');
 ...
 for(let i=0;i<5;i++){const x=4670+i*580;rect(c,x,380,170,210,'#3e465b');rect(c,x+15,356,135,24,'#536076');for(let j=0;j<5;j++)for(let k=0;k<3;k++)rect(c,x+18+j*30,408+k*45,13,24,'#859a9866',0);rect(c,x-4,596,179,18,'#566273');}
...
export function drawPickups(c:C,g:GameRuntime){for(const item of pickups){if(item.scene!==g.state.scene||g.state.collected.has(item.id))continue;const y=item.y+(g.reduced?0:Math.sin(g.time*3+item.x)*4);c.save();c.shadowColor=item.kind==='banana'?'#ffd85c':'#b8fb70';c.shadowBlur=12;if(item.kind==='banana')banana(c,item.x,y);else{c.translate(item.x,y);c.rotate(-.14);rect(c,-15,-9,30,18,'#b8ec86',2);text(c,'$',-5,6,15,'#376b41');}c.restore();}
 for(const p of g.collectibles.particles){c.save();c.globalAlpha=Math.min(1,p.life*1.5);if(p.label)text(c,p.label,p.x,p.y,14,p.color,'center');else ellipse(c,p.x,p.y,3,3,p.color);c.restore();}
}
export function drawInterior(c:C,w:number,h:number,g:GameRuntime,scene:GameScene){
 const grad=c.createLinearGradient(0,0,0,h);grad.addColorStop(0,'#172336');grad.addColorStop(1,'#403d51');c.fillStyle=grad;c.fillRect(0,0,w,h);c.save();c.translate(-g.camera.x,0);rect(c,20,250,1320,420,'#424856');rect(c,20,250,1320,25,'#696d73');rect(c,20,647,1320,70,'#8e827b');for(let i=0;i<13;i++)path(c,[[i*110,717],[i*110+25,648],[i*110+30,648],[i*110+5,717]],'#615d64',0);
 neon(c,'MONKE STOP',470,324,42,'#e9ca91');label(c,'OPEN ALL NIGHT. EVEN FOR BIG DREAMS.',640,352,'#bfbbb6');
 rect(c,68,386,90,267,'#263a3b');rect(c,76,399,74,148,'#61766d');text(c,'EXIT',84,426,20,'#d2e4bd');rect(c,137,559,7,16,'#c4bda0');
 ...
 for(let j=0;j<2;j++){rect(c,765,407+j*77,240,16,'#927963');for(let i=0;i<8;i++)rect(c,780+i*28,374+j*77,15,32,['#a8c489','#d8b375','#a39cb7'][i%3],1);}

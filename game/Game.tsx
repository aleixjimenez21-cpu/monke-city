...
import {localize,readLanguage,setLanguage,type Language} from './locale';
import {drawFinale,smooth} from './cinematic';
...
export default function Game(){
 ...
 useEffect(()=>{const value=readLanguage();setLanguage(value);updateLanguage(value);},[]);
 ...
 useEffect(()=>{
  const g=new GameRuntime(),scene=new GameScene(),c=canvas.current!,ctx=c.getContext('2d')!;engine.current=g;g.init();scene.load();scene.foreground=context=>drawFinale(context,g);scene.decorations=context=>drawStreetObjects(context,g);g.reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
  ...
  const frame=(now:number)=>{const dt=Math.min(.035,last?(now-last)/1000:0);last=now;
   ...
   scene.hidePlayer=g.state.phase==='ready'||g.state.phase==='loading'||g.state.phase==='ending'||(g.state.phase==='cinematic'&&g.phaseTime>=9.3);scene.playerOpacity=g.state.phase==='cinematic'?1-smooth((g.phaseTime-8.7)/.6):1;
 ...
 const move=(direction:string)=>(e:React.PointerEvent<HTMLButtonElement>)=>{e.preventDefault();e.currentTarget.setPointerCapture(e.pointerId);keys.current.add(direction);};const release=(direction:string)=>()=>keys.current.delete(direction);
 const active=['playing','arriving','dialogue','travel','ending'].includes(view.phase);
 return localize(<main className={`game ${mobile?'touch-mode':''} phase-${view.phase} scene-${view.scene}`}><canvas ref={canvas} aria-label={mobile?'Monke City. Use the on-screen arrows to move and jump. Tap interaction buttons.':'Monke City. Move with A and D, jump with Space, interact with E.'}/><div className="film-grain"/>
 <div className="brand">RICHMONKE<span> $RICH</span></div><div className="top-actions"><div className="language-switch" aria-label="Language / Idioma">{(['en','es'] as const).map(value=><button key={value} aria-pressed={language===value} onClick={()=>{setLanguage(value);updateLanguage(value);}}>{value.toUpperCase()}</button>)}</div><div className="social-dropdown"><button className="social-toggle" aria-expanded={social} aria-controls="social-links" onClick={()=>setSocial(open=>!open)}>COMMUNITY</button>{social&&<div id="social-links" className="social-menu"><SocialButtons compact/></div>}</div><button onClick={()=>{try{const g=engine.current;if(g)setMuted(g.audio.toggle());}catch{engine.current?.tell('Sound is unavailable in this browser.');}}} aria-label={muted?'Turn sound on':'Mute sound'}>{muted?'◌':'◉'} <span>SOUND {muted?'OFF':'ON'}</span></button><button className="menu-button" aria-label="Open game menu" onClick={()=>setMenu(true)}>☰</button></div>
 {view.phase==='loading'&&<div className="loading"><div className="load-bars">▰ ▰ ▰</div><span>LOADING MONKE CITY...</span></div>}
 {view.phase==='ready'&&<section className="start-screen cinematic-landing"><span className="eyebrow">A RICHMONKE STORY · AIM FOR THE CROWN.</span><h1><span>FROM ZERO TO</span><em>RICH MONKE</em></h1><p>One city. One monke. One mission.</p><button className="primary" onClick={()=>act(g=>g.start(g.hasSave))}>PLAY THE GAME <span>↗</span></button>{view.hasSave&&<button className="text-button fresh-start" onClick={()=>act(g=>g.start(false))}>Start a fresh journey</button>}<SocialButtons compact/><div className="start-note">01 / MONKE CITY</div></section>}
 {active&&view.phase!=='ending'&&<GameHUD mobile={mobile} anchor={engine.current&&view.near?Math.max(12,Math.min(88,100*(view.near.x-engine.current.camera.x)/engine.current.viewWidth)):50} view={view} onInteract={()=>act(g=>g.interact())}/>}
 ...
 {view.phase==='cinematic'&&<div className="cinematic-overlay"><div className="letterbox top"/><div className="letterbox bottom"/>{view.phaseTime<6&&<div className="call-subtitles" aria-live="polite"><span className="eyebrow">UNKNOWN CALLER</span><p>{view.phaseTime<3?'Your ride is here.':"It's time to get rich."}</p></div>}</div>}
 ...
 <footer><div className="district"><i/><span>{view.scene==='stop'?'MONKE STOP / OPEN ALL NIGHT':view.x>3050?'CROWN TOWER OVERLOOK':'MONKE MOTEL / OUTSKIRTS'}</span></div>{view.phase==='playing'&&!mobile?<div className="controls-hint"><kbd>A</kbd><kbd>D</kbd><span>MOVE</span><kbd>SPACE</kbd><span>JUMP</span><kbd>E</kbd><span>INTERACT</span></div>:<span className="build-label">EVERY MONKE STARTS SOMEWHERE.</span>}<span className="chapter-number">01 <span>/ THE OUTSKIRTS</span></span></footer>
 ...
 <GameMenu mobile={mobile} open={menu} view={view} onClose={()=>setMenu(false)} onRestart={()=>{act(g=>g.reset());setMenu(false);}}/></main>);
}

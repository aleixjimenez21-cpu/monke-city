...
export default function Game(){
 ...
 useEffect(()=>registerGameTools(()=>{const g=engine.current;return g?{phase:g.state.phase,mission:g.mission.current.id,scene:g.state.scene,x:Math.round(g.player.x),netWorth:g.state.netWorth,bananas:g.collectibles.count(g.state),objectives:[...g.state.flags]}:{phase:'loading'};},()=>{const g=engine.current;if(g?.state.phase!=='ready')throw new Error('Wait for the title screen before starting');g.start(g.hasSave);return{phase:g.state.phase};}),[]);
 ...
 const active=['playing','arriving','dialogue','travel','ending'].includes(view.phase),intro=Math.min(2,Math.floor(view.phaseTime/1.5));
 return <main className={`game phase-${view.phase} scene-${view.scene}`}><canvas ref={canvas} aria-label="Monke City. Move with A and D, jump with Space, interact with E. Find five bananas, discover Monke Stop, and reach Crown Tower."/><div className="film-grain"/>

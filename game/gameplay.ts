...
export class PlayerController {
 ...
 update(p:Player,scene:District,dt:number,direction:number){const ox=p.x,oy=p.y;p.update(dt,direction);const jumped=this.jump.update(p,dt);this.collision.resolve(p,ox,oy,platforms.filter(s=>s.scene===scene));if(scene==='stop')p.x=Math.max(80,Math.min(1250,p.x));return jumped;}
}
...
export class InteriorSystem {
 streetX=1405;
 enter(state:GameState,p:Player,camera:Camera){this.streetX=p.x;state.scene='stop';p.x=170;p.y=662;p.vy=0;p.velocity=0;camera.x=0;camera.worldWidth=1360;camera.descent=0;}
 exit(state:GameState,p:Player,camera:Camera){state.scene='street';p.x=this.streetX;p.y=662;p.vy=0;p.velocity=0;camera.worldWidth=7600;camera.x=Math.max(0,p.x-400);}
}

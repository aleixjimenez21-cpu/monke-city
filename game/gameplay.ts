...
export class ProgressStore {
 ...
 load(state:GameState,p:Player){try{const raw=localStorage.getItem(this.key);if(!raw)return false;const d=JSON.parse(raw);if(![2,3,4].includes(d.version)||!Array.isArray(d.collected)||!Array.isArray(d.flags)||!Number.isFinite(d.x))return false;
  ...
  const validFlags=['stop-entered','stop','left-stop','rich','jumped','doubter','map','terminal','tv','owner','challenge','complete','early-monke','moved','jump-used','interacted','ride-complete'];state.flags=new Set(d.flags.filter((id:unknown)=>validFlags.includes(String(id))));if(d.version===2){state.flags.delete('complete');state.flags.delete('challenge');if(state.flags.has('terminal')){state.flags.add('stop');state.flags.add('stop-entered');state.flags.add('left-stop');}}state.secrets=new Set(Array.isArray(d.secrets)?d.secrets.filter((id:unknown)=>encounters.some(e=>e.kind==='secret'&&e.id===id)):[]);state.vote=(voteOptions as readonly unknown[]).includes(d.vote)?d.vote:null;state.playedSeconds=Number.isFinite(d.playedSeconds)?Math.max(0,d.playedSeconds):0;p.x=Math.max(90,Math.min(mission001.endX,d.x));state.scene='street';if(d.version<4){state.collected.clear();state.netWorth=7;state.flags.clear();p.x=mission001.startX;state.playedSeconds=0;}else if(state.flags.has('stop')){state.flags.add('left-stop');p.x=Math.max(1350,p.x);}return true;
 }catch{return false;}}

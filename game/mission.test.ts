...
if(inspect){for(const id of ['map','tv','terminal','owner','rich']){const e=encounters.find(e=>e.id===id)!;walk(g,e.x);for(let i=0;i<150;i++)g.update(dt,0);g.interact();assert.equal(g.dialogue.encounter?.id,id);dialogue(g);}awaitVote(g);}
...
const fast=run(false),curious=run(true);assert(pickups.length>=3&&pickups.length<=5);assert.equal(encounters.filter(e=>e.story).length,1);assert.equal(encounters.filter(e=>e.scene==='stop'&&e.kind!=='door').length,5);
...
console.log('PASS: complete physical playthrough, both skipping and inspecting all five shop interactions; four rewards; one required NPC; short dialogue; reachable jumps; 10–15 second final run; persistence, legacy-save restart and optional vote.');
...
// Store readings are optional and return automatically without a large modal.
for(const id of ['map','tv','terminal','owner','rich']){const e=encounters.find(e=>e.id===id)!;const r=new GameRuntime();r.state.scene='stop';r.player.x=e.x;r.player.grounded=true;r.setPhase('playing');r.interact();assert.equal(r.dialogue.encounter?.id,id);let elapsed=0;while(r.state.phase==='dialogue'&&elapsed<6){r.update(dt,0);elapsed+=dt;}assert.equal(r.state.phase,'playing',id+' returns to gameplay');assert(r.state.flags.has(e.flag!));assert(elapsed<=4.3,id+' stays brief');}
const held=new GameRuntime();held.state.scene='stop';held.player.x=690;held.setPhase('playing');held.interact();held.inspectionHeld=true;for(let i=0;i<600;i++)held.update(dt,0);assert.equal(held.state.phase,'dialogue');held.inspectionHeld=false;for(let i=0;i<400;i++)held.update(dt,0);assert.equal(held.state.phase,'playing');

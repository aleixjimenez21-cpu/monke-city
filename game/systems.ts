...
export type Phase = 'loading' | 'intro' | 'ready' | 'arriving' | 'playing' | 'dialogue' | 'ending' | 'travel';
export const missions = [mission001];
export class GameState { phase: Phase = 'loading'; netWorth = 7; paused = false; signRead = false; scene: District = 'street'; flags = new Set<string>(); collected = new Set<string>(); secrets = new Set<string>(); vote: string | null = null; playedSeconds = 0; }
export class Player {
...
export class MissionSystem { current = missions[0]; complete = false; next(flags:Set<string>) { return this.current.objectives.find(o=>!flags.has(o.id)); } canChallenge(flags:Set<string>){return this.current.objectives.slice(0,7).every(o=>flags.has(o.id));} shouldEnd(x: number, flags: Set<string>) { return x >= this.current.endX && this.canChallenge(flags)&&flags.has('challenge'); } }

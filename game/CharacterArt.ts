...
export class CharacterArt {
 ...
 draw(c:C,p:Player,t:number,reduced:boolean,fallback:HTMLImageElement|null){
  ...
  const warm=this.interior||Math.abs(p.x-1120)<250,neon=p.x>2990||Math.abs(p.x-2200)<150;c.shadowColor=warm?'#f8b96c':neon?'#b8fb70':'#94a5e2';c.shadowBlur=warm||neon?5:2;c.shadowOffsetX=-2;
